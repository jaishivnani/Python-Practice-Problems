...
# def string(str):
#     char = str[0].lower()         #Storing 1st character in lower for using it later.
#     str = str.replace(char,'$')   #Doing str = str.replace(char,'$') for replacing 1st character and its occurences to $.
#     return str[0].upper() + str[1:]       #Returning char stored + str[1:](String starting from position 1 to ...)

# def string(str):
#     return str[0] + str[1:].replace(str[0].lower(),'$') # - In One Line
